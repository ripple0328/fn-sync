...
> The current version is `0.10.3`. Discovery, authorization, folder browsing,
...
- `fn-sync-0.10.3-1-any.pkg.tar.zst`: Arch/pacman system package.
- `fn-sync-omarchy-plugin-0.10.3.tar.gz`: self-contained Omarchy plugin.
- `fn-sync-omarchy-bundle-0.10.3.tar.gz`: portable bundle containing both
...
port responds. The panel uses one atomic **Test and save** action; failed tests leave
the previously saved connection unchanged.
