...
> 当前版本为 `0.10.3`。真实 fnOS 上的发现、授权、文件夹浏览和大型同步已经
...
- `fn-sync-0.10.3-1-any.pkg.tar.zst`：Arch/pacman 系统包。
- `fn-sync-omarchy-plugin-0.10.3.tar.gz`：自包含 Omarchy 插件归档。
- `fn-sync-omarchy-bundle-0.10.3.tar.gz`：包含前两者和安装器的便携包。
...
实际响应后才会自动填写。面板使用一个原子的“测试并保存”操作；测试失败时，
已有连接保持不变。自定义
...
- 92 个自动化测试覆盖真实 WebDAV 双向/单向传输、完整 CLI 连接与任务生命周期、
  流式进度与静默超时、后台调度、Secret Service、局域网发现、一键安装器沙箱、
  已打包控制器启动、发布源码完整性、打包/发布契约、双语文档以及独立插件测试；
  Python 行覆盖率与分支覆盖率的综合门槛为 85%，当前测试结果为 87%。
